...
int DSATUR_::DSATUR_h(C_Graphe &GC){
	...
	for(int i=0 ; i<GC.nb_sommets ; i++){coloration_courante[i] = -1;dsat_courant[i] = 0;}
	...
	while(cpt_boucle<GC.nb_sommets){
		...
		for(int j=0 ; j<G.nb_sommets ; j++){
			...
			if(coloration_courante[i] == -1){
				...
				}else{if(dsat_courant[i] == choix_dsat){//En cas d'égalité on le rajoute
					candidats[size_candidats] = i;
					size_candidats++;
				}}
			}
		...
		for(int i=1 ; i<=solution_courante+1 ; i++){
			...
			for(int k=0 ; k<GC.degre[choix_som] ; k++){
				if(coloration_courante[GC.sommets_voisins_bis[choix_som*GC.nb_sommets+k]] == choix_coul){
					ajout = false;
					break;
				}
			}
		...
		for(int k=0 ; k<GC.degre[choix_som] ; k++){
			...
			for(int l=0 ; l<GC.degre[tmp] ; l++){
				if(coloration_courante[GC.sommets_voisins_bis[tmp*GC.nb_sommets+l]] == choix_coul){
					ajout = false;
					break;
				}
			}
...
void DSATUR_::DSATUR_preprocessing(){
	...
	for(int i=0 ; i<G.nb_sommets ; i++){deleted_vertices[i] = 0;}
#ifdef DEBUG
	cout<<"RECHERCHE DOMINE\n";
#endif
	//On retire potentiellement les dominated vertices
	for(int i=0 ; i<G.nb_sommets ; i++){
		for(int j=i ; j<G.nb_sommets ; j++){if(i!=j && G.degre[j] > G.degre[i]){
			bool dominated = true;
			for(int k=0 ; k<G.degre[i] ; k++){
				bool found = false;
				for(int l=0 ; l<G.degre[j] ; l++){
					if(G.sommets_voisins_bis[i*G.nb_sommets+k] == G.sommets_voisins_bis[j*G.nb_sommets+l]){found=true;break;}
				}
	...
	for(int i=0 ; i<G.nb_sommets ; i++){
		if(G.degre[i] < (LB-1)){if(deleted_vertices[i] == 0){new_nb_sommets--;}deleted_vertices[i] = 1;}
	}
	...
	cout<<G.nb_sommets-new_nb_sommets<<" sommet(s) a enlever\n";
		...
		Gnew.adj = (bool *)malloc(Gnew.nb_sommets*Gnew.nb_sommets*sizeof(bool));
		Gnew.sommets_voisins_bis = (int *)malloc(sizeof(int )*Gnew.nb_sommets*Gnew.nb_sommets);
		...
		int indice_i=0;
		for(int i=0 ; i<G.nb_sommets-1 ; i++){if(deleted_vertices[i] == 0){
			int indice_j=indice_i+1;
			for(int j=i+1 ; j<G.nb_sommets ; j++){if(deleted_vertices[j] == 0){
				Gnew.adj[indice_i*Gnew.nb_sommets+indice_j] = G.adj[i*G.nb_sommets+j];
				Gnew.adj[indice_j*Gnew.nb_sommets+indice_i] = G.adj[i*G.nb_sommets+j];
				if(G.adj[i*G.nb_sommets+j]){
					Gnew.nb_aretes++;

					Gnew.sommets_voisins_bis[(indice_i*Gnew.nb_sommets)+Gnew.degre[indice_i]] = indice_j;
					Gnew.degre[indice_i]++;

					Gnew.sommets_voisins_bis[(indice_j*Gnew.nb_sommets)+Gnew.degre[indice_j]] = indice_i;
					Gnew.degre[indice_j]++;
				}
				indice_j++;
			}}
...
int DSATUR_::DSATUR_algo(C_Graphe &G_param, double time_param, int regle_input, int LB_input, int UB_input){
	...
	for(int i=0 ; i<G.nb_sommets ; i++){reorder[i] = i;}
	...
	for(int i=0 ; i<G.nb_sommets ; i++){F_V1[i] = 1;F_V2[i] = 1;}
	F = (bool *)malloc(sizeof(bool)*G.nb_sommets*G.nb_sommets);
	for(int i=0 ; i<G.nb_sommets*G.nb_sommets ; i++){F[i] = true;}
	s = (int *)malloc(sizeof(int)*G.nb_sommets*G.nb_sommets);
	for(int i=0 ; i<G.nb_sommets*G.nb_sommets ; i++){s[i] = 1;}
...
void DSATUR_::DSATUR_algo_rec(int profondeur){
	...
	if(profondeur == G.nb_sommets){//Feuille
		...
		if(solution_courante < UB){
			...
			#ifdef show_UB
			#ifdef activate_time
			cout<<"Amélioration solution courante : "<<solution_courante<<" "<<nombre_noeuds<<" "<<time<<"\n";
			#else
			cout<<"Amélioration solution courante : "<<solution_courante<<" "<<nombre_noeuds<<"\n";
			#endif
			#endif
...
//			}
		...
		//Recursion
		for(int i=1 ; i<=solution_courante+1 ; i++){if(F[choix_sommet*G.nb_sommets+i]){
			#ifdef DEBUG
			...
			if(solution_courante < UB){
				...
				mise_a_jour_dsat(choix_sommet,i,liste_changements_F,&size_changements_F);
